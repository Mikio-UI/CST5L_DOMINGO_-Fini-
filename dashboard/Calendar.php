...
<head>
    ...
    <link rel="stylesheet" href="/Fini/css/Calendar.css">
    <style>
        :root.light-mode {
            --bg:     #f0f2f8;
            --panel:  #ffffff;
...
<body>
<script>(function(){if(window.self===window.top){window.location.replace('/Fini/shell.php?page=calendar');}}());</script>
...
<script>
    ...
    document.getElementById('saveEvent').addEventListener('click', async () => {
        ...
        try {
            const res  = await fetch('/Fini/api/Tasks.php', {
                method:  'POST',
...
async function refreshSidebar() {
    try {
        const res  = await fetch('/Fini/dashboard/Calendar.php?ajax=sidebar');
        const data = await res.json();
